...
- Требуется включить в Bios'е (для процессоров Intel) технологию виртуализации VT-d
  ...
  У меня на HuananZhi X79 ZD3 она находится во вкладке **Chipset** - **North
  Bridge** - **IOH Configuration** - **Intel VT for Directed I/O Configuration**
  и включаем **Intel VT-d**
...
- libvirt - Позволяет обычному пользователю (через PolicyKit) подключиться к
  демону libvirtd, не запрашивая пароль root
...
отредактировать `qemu.conf`, и дать привилегии qemu читать данный iso.
...
> Пояснение: Делается это для того чтобы можно было пробросить свою видеокарту
> в VM
...
Выбираем созданную ВМ из списка и жмём "Свойства подключения" который находятся
в тулбаре менеджера "Правка"
...
С прописанными в `/etc/hosts` от [StevenBlack](https://github.com/StevenBlack/hosts)
из [данного моего гайда](/posts/hosts-file-by-stevenblack), dnsmasq возмущается
на данные строки
...
Решил это просто отредактировав файл hosts закомментировав (НЕ удалять) эти
указанные номером строки
...
Столкнулся с данной ошибкой, из-за того что место в котором расположено iso
...
Решилось путём редактирования конфига `/etc/libvirt/qemu.conf`. Раскоментируем
...
В Arch Linux при настроенным тайловым оконным менеджере всё стало увеличенным
включая терминал

Происходит это когда я юзаю дисплей Virtio + OpenGl ускорением

При создании ВМ с Arch Linux и любом окружении WM/DE скейлит текст в терминале
alacritty (в конфиге alacritty размер шрифта выставлен 14)

Текст выглядит больше обычного

## Проблема: Использовании picom в VM

В настройках виртуального оборудования во вкладке Видео выставлено Модель: Virtio
с включенным 3D ускорением. При использовании моего picom config'а как композитора
в окружениях таких как LXDE, LXQT и т.п перемещение окон идёт с отставанием с
заметным input лагом
...
Ошибка создания снимка: Операция не поддерживается: внутренние снимки виртуальной
машины с микропрограммой на основе pflash не поддерживаются
...
- Решение для гостевой Linux машине: создавать снимок гостя можно только в
  выключенном состоянии (UEFI)

- Решение для гостевой Win10 машине: Смог исправить это отредактировав xml файл
  в данной строке вместо **type="pflash"** написать **type="rom"**
...
После переустановки Арча на хосте я импортировал qcow образ и обнаружил что на
нём вместо появления grub и запуска я получаю UEFI Interactive Shell
...
заново установить grub в `/boot/efi` и обновить конфиг grub
...
**Решением** было сразу при редактировании вставить заполненную строку с
содержимым т.е где троеточие должны быть вставлены переменные
...
**Убедитесь** чтобы в virt-manager'е было добавлено оборудование - Канал Spice
с типом уст-ва Агент SPICE (spicevmc). Если оборудование у вас есть тогда просто
перезагружаем гостевую машину, после скачивания пакета
...
в терминале, можно также её добавить в автозапуск файла `~/.xprofile` (Для
DM) или `~/.xinitrc` (запуск с TTY)
...
- Путь в гостевой ОС: Qemu (место в гостевой VM используемое для идентификации
  общего каталога, который необходимо смонтировать в гостевой ОС)

После добавления уст-ва загружаемся в VM и создаём каталог для монтирования
общей папки
...
- Драйвер: virtio-9p
- Путь на хосте: /home/user/Shared (т.е то место где мы добавляем файлы с
  которыми хотим поделится чтобы гость их видел)
- Путь в гостевой ОС: host0 (идентификация общего каталога т.е mount tag, называть
  можно как угодно)
...
Если вы ДЕЙСТВИТЕЛЬНО не знаете что делаете, не делайте этого. Потому что virsh
edit имеет проверка синтаксиса, чтобы выявлять некоторые проблемы. Совсем как
visudo делает.
...
1. Активный снимки в Virt Менеджере не поддерживаются если включён UEFI

   [Источник](https://unix.stackexchange.com/questions/663372/error-creating-snapshot-operation-not-supported-internal-snapshots-of-a-vm-wit)

   Решение: Делать снимки вм только в выключенном состоянии

2. Нету разрешений на доступ к папке /home/anix/Shared в Virt Manager используя
   virtio-9p
