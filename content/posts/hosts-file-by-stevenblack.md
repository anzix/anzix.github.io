...
Для избавления от рекламы, реферальных ссылок и всяких всплывающих scam сайтов
есть обновляемый hosts файл
...
> В конце `>/dev/null` значит что команда будет выполняться без вывода подробной
> информации (полезная команда которая может пригодится для написания конфигов)

## Обновление универсального hosts файла

Я для себя написал небольшую shell функцию которая производит сравнение с
источника репозитория StevenBlack и моим текущим hosts файлов и удаляет/добавляет
необходимое в него.
