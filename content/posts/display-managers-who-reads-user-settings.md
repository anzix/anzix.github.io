...
- Ручной запуск из TTY - Подхватывает настройки от выбранного `$SHELL`
  ...
  xinitrc подхватывается переменной `$XINITRC` прописанной в `~/.zprofile` и может
  находится где угодно

- LightDM - подхватывает настройки независимо от `$SHELL`, но необходимо
  наличие `~/.xprofile`
...
- LXDM - подхватывает настройки независимо от **$SHELL** как LightDM, но
  необходимо наличие `~/.xprofile`
...
- [Arch Wiki Environment variables - Per user Xorg X11](https://wiki.archlinux.org/title/Environment_variables#Per_user)
