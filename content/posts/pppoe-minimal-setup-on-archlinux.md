+++
title = "Интернет подключение по проводу без роутера (PPPoE)"
date = 2023-03-09
draft = false
[taxonomies]
categories = []
...
Мне помог [этот гайд из группы ВК по Арчу](https://vk.com/topic-129498031_39721608).
Хотя он изначально почему-то не работал, странно

А вообще этот метод описан в [Arch Wiki по netctl](https://wiki.archlinux.org/title/netctl#Wired_2)

## Настройка и запуск

Скачиваем пакет [netctl](https://archlinux.org/packages/core/any/netctl/) с
зависимостями [openresolv](https://archlinux.org/packages/core/any/openresolv/)

> Если вы в офлайне тогда качаем эти 2 пакета (нажав **Download From Mirror**)
> с устройства у которого есть инет и передаём по usb и устанавливаем используя
> `sudo pacman -U`
...
В `Interface` меняем на наш из команды `ip link` в терминале
...
У меня немного по началу долго грузил вкладки минуты 2, но потом перестало и
начал грузить мгновенно
