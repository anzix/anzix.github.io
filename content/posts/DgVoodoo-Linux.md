...
Старая игра на DirectD3D 1-8 преобразовывается DgVoodoo в более современный DirectD3D
9/10/11 и DXVK в свою очередь переводит данные api в Vulkan
...
- В WM (Оконных менеджерах) нету никаких проблем, особенно с использованием композитора
  picom и переключении рабочих столов (тестировалось на i3wm)
...
Прописываем путь до своего префикса (слепок windows) и устанавливаем игру прописывая
путь к setup.exe
...
Для этого копируем 4 dll'ки внутри из [скачанного dgvoodoo](http://dege.freeweb.hu/dgVoodoo2/dgVoodoo2/)
**/dgVoodoo2_79_3/MS/x86/** в папку с игрой
...
> Или можно (используя default префикс) открыв через wine (не указывая префикс)
> файл dgVoodooCpl.exe и изменять там нужные для вас параметры и потом после
> сделанных изменений (нажав **Apply**) копируете модифицируемый **dgVoodoo2.conf**
> в папку игры

Перед запуском игры необходимо дать указание wine использовать нативные dll'ки
(т.е которые используются от dgvoodoo внутри игры) указав данную переменную.
